...
namespace Tienda_Tres_Monosabios
{
    ...
    public partial class EditarCliente : Form
    {
        ...
        public EditarCliente(int IdCliente, string NombreCliente, int Nit, int DPI, string Direccion, int Telefono, DateTime FechaNacimiento, int idCiudad)
        {
            InitializeComponent();
            Idclient = IdCliente;
            textBoxNombreCliente.Text = NombreCliente;
            textBoxDPI.Text = Convert.ToString(DPI);
            textBoxDireccion.Text = Direccion;
            comboBoxCiudad.SelectedValue = idCiudad.ToString();
            textBoxTeléfono.Text = Convert.ToString(Telefono);
            textBoxNit.Text = Convert.ToString(Nit);
        ...
        private void buttonEditarCliente_Click(object sender, EventArgs e)
        {
            ...
            ConexionBD editarCliente = new ConexionBD();
            editarCliente.EditarCliente(Idclient, cliente);
            this.Close();
        }
    }
}
